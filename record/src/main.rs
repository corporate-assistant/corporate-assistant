...
fn main() -> Result<(), String> {
    ...
    if record_args
        .database
        .to_str()
        .expect("Improper name of database given")
        == "all"
    {
        println!(
            "DataBase: {} . Please say: {}",
            "train.csv", record_args.text
        );
        myrecorder
            .record_and_store(&std::path::PathBuf::from("train.csv"), &record_args.text)
            .unwrap();
        println!("DataBase: {} . Please say: {}", "dev.csv", record_args.text);
        myrecorder
            .record_and_store(&std::path::PathBuf::from("dev.csv"), &record_args.text)
            .unwrap();
        println!(
            "DataBase: {} . Please say: {}",
            "test.csv", record_args.text
        );
        myrecorder
            .record_and_store(&std::path::PathBuf::from("test.csv"), &record_args.text)
            .unwrap();
    } else {
        println!(
            "DataBase: {:?} . Please say: {}",
            record_args.database, record_args.text
        );
        myrecorder
            .record_and_store(&record_args.database, &record_args.text)
            .unwrap();
    }
