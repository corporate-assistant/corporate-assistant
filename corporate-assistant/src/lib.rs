pub mod interpreter {
  ...
  impl Intents {
   pub fn register_action(&mut self, keys : Vec<String>, action : Rc<dyn CorporateAction> ) -> Result<(),String> {
      for key in keys {
   ...
   pub fn get_action(&self, key : &str) -> Result< Rc<dyn CorporateAction> ,String> {
     if self.intents.contains_key(key) {Ok(self.intents[key].clone())}  else {Err(format!("Action \"{}\" not recognized",key) as String)}
   }
