...
use fltk::{app, button::Button, frame::Frame, prelude::*, text::TextEditor, text::TextBuffer, window::Window };
...
pub fn run(commands_list : Vec<&str>,candidate_label : &str) -> String {
    ...
    let vpack = fltk::group::Pack::default().with_size(600, 600).center_of(&wind);
    ...
    for l in commands_list.iter() {
        let mut label_button = Button::default()
            .with_size(0,50)
            .with_label(l);
    ...
    let mut hpack = fltk::group::Pack::default().with_size(600, 50).above_of(&vpack,0);
    ...
    let mut custom_label = Button::default()
        .with_size(50,0)
        .with_label("Ok");
    hpack.end();
    let frame = Frame::default()
        .with_size(600,50)
        .with_label("Custom label:").above_of(&hpack,0);
